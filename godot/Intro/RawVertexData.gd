...
func animate_vertices() -> void:
	...
	for i in range(0, indices.size(), 3):
		...
		tween.interpolate_property(vertices[i1], "position", vertices[i1].position, vertex_end_positions[i1], anim_time, Tween.TRANS_SINE, 2)
		tween.interpolate_property(vertices[i2], "position", vertices[i2].position, vertex_end_positions[i2], anim_time, Tween.TRANS_SINE, 2)
		tween.interpolate_property(vertices[i3], "position", vertices[i3].position, vertex_end_positions[i3], anim_time, Tween.TRANS_SINE, 2)
		tween.start()
		yield(tween, "tween_all_completed")
...
func do_cull() -> void:
	for i in range(culled_indices.size()):
		tween.interpolate_property(vertices[culled_indices[i]], "modulate", Color.white, Color.red, 1)
	tween.start()
	yield(tween, "tween_all_completed")
	for i in range(culled_indices.size()):
		tween.interpolate_property(vertices[culled_indices[i]], "modulate", Color.red, Color.transparent, 1)
	tween.start()
	yield(tween, "tween_all_completed")
