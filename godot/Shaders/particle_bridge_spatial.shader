...
void vertex() {
	...
	descend_factor = step(0.0, INSTANCE_CUSTOM.z);
	...
	// Mix y offset based on the descend_factor property. Note that "descend_factor" is
	// assigned either to 0 or 1 by using step at aline 43.
	VERTEX.y += mix(y_descend, y_ascend, descend_factor);
	...
	VERTEX.xz *= mix(xz_scale_descend, xz_scale_ascend, descend_factor);
}
