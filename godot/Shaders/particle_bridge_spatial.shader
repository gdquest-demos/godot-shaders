...
void vertex() {
	...
	VERTEX.y += sin(TIME + float(rand_from_seed(INSTANCE_ID) * 133.0)) * pow(INSTANCE_CUSTOM.y, 3.0) * oscillation_size ;
	normalized_distance = INSTANCE_CUSTOM.y * (step(INSTANCE_CUSTOM.y, .99));
...
void fragment(){
	...
	vec3 emission_ascend = emission_color.rgb * clamp(
		pow(smoothstep(0.0, 0.7,normalized_distance), 4.0),
		0.0, 1.0
		);
