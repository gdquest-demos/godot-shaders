...
void fragment() {
	...
	if (rim_light_color.r > 0.0 || rim_light_color.g > 0.0 || rim_light_color.b > 0.0) {
		...
		float view_dot_product = dot(
				normalize(NORMAL + vec3(0, rim_normal_offset_y, rim_normal_offset_x)), 
				VIEW);
				
		float rim_value = pow(1.0 - view_dot_product, rim_fresnel_power);
		float hard_rim = step(RIM_SHARPNESS, rim_value);
		float soft_rim = smoothstep(RIM_SOFTNESS_MIN, RIM_SOFTNESS_MAX, rim_value);
		
		vec3 out_rim_light = vec3(
				mix(hard_rim, soft_rim, rim_light_softness)) * rim_light_color.rgb;
	...
	if (metalness > 0.0) {
		...
		metalness_value = clamp(
				metalness_value, 
				dark_metalness_color.rgb, 
				light_metalness_color.rgb);
	...
	if (specular_size > 0.0) {
		float soft_specular = smoothstep(
				SPECULAR_SOFT_MIN, 
				SPECULAR_SOFT_MAX, 
				specular * specular_size);
				
		float hard_specular = smoothstep(
				SPECULAR_HARD_MIN, 
				SPECULAR_HARD_MAX, 
				specular * specular_size);
		
		vec3 specular_out = mix(
				hard_specular, 
				soft_specular, 
				specular_softness) * specular_color.rgb;
	...
	if (anisotropy_specular_strength > 0.0) {
		...
		float anisotropy_specular_hotspot = smoothstep(
				ANISOTROPY_HOTSPOT_MIN, 
				ANISOTROPY_HOTSPOT_MAX, 
				specular * anisotropy_specular_width);
	
		float spottiness_anisotropy_noise_value
				= (texture(spottiness_anisotropy_noise, vec2(UV.x, 0)).r - 0.5)
				* anisotropy_specular_contrast + anisotropy_specular_brightness;
		...
		float anisotropy_value = 
				mix(sharp_anisotropy_value, soft_anisotropy_value, specular_softness);
